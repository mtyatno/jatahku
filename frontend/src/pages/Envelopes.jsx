...
function CreateModal({ onClose, onCreated, editing }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setSaving(true);
    const data = { name, emoji, budget_amount: Number(budget), is_rollover: rollover, is_personal: isPersonal };
...
function EnvelopeCard({ env, onEdit, onDelete }) {
  ...
  return (
    <div className="card group hover:border-brand-200 transition-all">
          ...
          {shared.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {shared.map(env => <EnvelopeCard key={env.id} env={env} onEdit={setEditing} onDelete={handleDelete} />)}
              </div>
            </div>
          )}
          {personal.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {personal.map(env => <EnvelopeCard key={env.id} env={env} onEdit={setEditing} onDelete={handleDelete} />)}
              </div>
            </div>
          )}
        </>
