...
async def link_telegram_account(req: LinkTelegramRequest, db: AsyncSession = Depends(get_db)):
    r = await _redis()
    ...
    if existing_user and str(existing_user.id) != user_id:
        raise HTTPException(status_code=400, detail="Akun Telegram sudah terhubung ke akun lain")
    ...
    await r.close()
    return {"status": "linked", "user_name": user.name}
...
async def claim_telegram_account(req: ClaimTelegramRequest, db: AsyncSession = Depends(get_db)):
    r = await _redis()
